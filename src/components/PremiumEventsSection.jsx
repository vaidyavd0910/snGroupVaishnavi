...
const PremiumEventsSection = () => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      // Handle both success/data structure and direct data
  ...
  const applyFilters = () => {
    ...
    if (filters.dateFilter !== 'all') {
      filtered = filtered.filter(event => event?.status === filters.dateFilter);
    }

    // Location filter
    if (filters.locationFilter !== 'all') {
      filtered = filtered.filter(event => 
        event?.location?.toLowerCase().includes(filters.locationFilter.toLowerCase())
      );
    }
              ...
              >
                  ...
                  <div className="absolute bottom-4 left-4 bg-white bg-opacity-90 backdrop-blur-sm rounded-xl px-3 py-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm font-semibold text-gray-900">
                        {format(event?.date || new Date(), "MMM d, yyyy")}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="flex space-x-3">
                    <button className="flex-1 bg-primary-500 text-white py-3 px-4 rounded-xl font-semibold hover:bg-primary-600 transition-colors duration-200 flex items-center justify-center space-x-2">
