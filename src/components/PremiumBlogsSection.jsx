...
const PremiumBlogsSection = () => {
  ...
  const sliderSettings = {
    ...
    infinite: true,
    speed: 800,
    slidesToShow: Math.min(4, filteredBlogs.length || 1), // prevent issue when blogs < 4
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 3000,
    pauseOnHover: true,
    arrows: true,
    responsive: [
      { breakpoint: 1280, settings: { slidesToShow: Math.min(3, filteredBlogs.length || 1) } },
      { breakpoint: 1024, settings: { slidesToShow: Math.min(2, filteredBlogs.length || 1) } },
